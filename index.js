...
//Dictionaries containing the conversions between V-Grade to Font and viseversa
var VToFont = {
  "V0": "4",
  "V1": "5",
  "V2": "5+",
...
const ConvertVGradeHandler = {
  ...
  handle(handlerInput) {
    //get the custom slot value from the command (the V grade)
    var slotValue = Alexa.getSlotValue(handlerInput.requestEnvelope, 'VGrade');

    //output (converted V grade to Font)
...
const ConvertFontGradeHandler = {
  ...
  handle(handlerInput) {
    //get custom slot value (the Font grade)
    var slotValue = Alexa.getSlotValue(handlerInput.requestEnvelope, 'FontGrade');
    var speechOutput= "";

    //Sometimes when I say "7A plus" the NLU interprets it as "78 plus" 
    // so I made a quick exception for that.
    ...
    //output (converted Font grade to V grade)
